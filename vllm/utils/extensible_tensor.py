...
class _VirtualBuffer:
    ...
    def _create_physical(self, size: int):
        driver = self._driver
        if self._shareable:
            try:
                return driver.create(size, self.device_index, shareable=True)
        ...
        return driver.create(size, self.device_index)
    ...
    def _map_chunk_at(self, offset: int, size: int) -> None:
        ...
        try:
            handle = self._create_physical(size)
        except RuntimeError:
...
class ExtensibleKVCacheBuffers:
    ...
    def commit_rounding_overhead(self) -> int:
        """Worst-case bytes lost to per-segment granule rounding at commit."""
        return sum(
            buffer.num_segments * buffer.granularity for buffer, _ in self.buffers
        )
