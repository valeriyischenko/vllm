...
def test_get_kv_cache_config_one_worker():
    ...
    # extensible KV cache prefers a block-outermost layout for pages of
    # different sizes.
...
def test_validate_extensible_kv_cache_requires_v2_runner(explicit):
    """The extensible KV cache is a V2-model-runner feature. Explicit requests
    fail on V1; a default quietly steps down, so enabling by default does not
    ...
    config = SimpleNamespace(
        ...
        model_config=SimpleNamespace(enable_sleep_mode=False, runner_type="generate"),
    )
...
@pytest.mark.parametrize("explicit", [False, True])
def test_validate_extensible_kv_cache_with_connector_sleep_mode(explicit):
    """Sleep mode alongside a KV connector cannot use the extensible KV cache
    even on V2: waking remaps physical pages, invalidating the connector's
    memory registration."""
    cache_config = CacheConfig(enable_extensible_kv_cache=True if explicit else None)
    config = SimpleNamespace(
        cache_config=cache_config,
        ...
        model_config=SimpleNamespace(enable_sleep_mode=True, runner_type="generate"),
    )
