...
def test_engine_core_proc_instantiation_cuda_empty(monkeypatch: pytest.MonkeyPatch):
    ...
    def create_mock_executor(vllm_config):
        ...
        # The engine core resolves the KV cache layout from this RPC.
        mock_executor.collective_rpc.side_effect = lambda method, *a, **k: (
            [["LBNHC"]] if method == "get_supported_kv_cache_layouts" else [None]
        )
